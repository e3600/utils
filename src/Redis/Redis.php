...
class Redis
{
   ...
   * @param string $table  表名
   * @param string $column 字段名
   * @param int    $num    增量值，默认1, 也可以是负数值,相当于对指定字段进行加法操作
  ...
  /**
   * 为哈希表中的字段值加上指定递减值(支持整数和浮点数)
   *
   * @param string $table  表名
   * @param string $column 字段名
   * @param int    $num    递减值，默认1, 也可以是负数值,相当于对指定字段进行减法操作
   * @return int|float|bool  返回计算后的字段值,如果字段值不是数字值则返回false,如果哈希表不存在或字段不存在返回false
   */
  public static function hdec($table, $column, $num = 1)
  {
    $value = self::hget($table, $column);
    if (is_numeric($value)) { // 数字类型，包括整数和浮点数
      $value -= $num;
